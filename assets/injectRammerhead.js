document.waitForElement = function(selector) {
  ...
  document.waitForElement(`[class^="browser-tab-content"]`).then(async el => {
    ...
    window.addEventListener("requestSpeedInfo", async (event) => {
      const isActive = rhSession.speedTicketActive || false;

      const speedTicketSettings = (isActive? (await fetch(`/api/speedTicketSettings?id=${rhSession.id}`).then(async r => {
        try {
      ...
      const speedTicketUsage = (isActive? (await fetch(`/api/speedTicketApply?id=${rhSession.id}&code=${localStorage.getItem("rh-speedticket")}`).then(async r => {
        try {
          return JSON.parse(await r.text());
        } catch {
          return null;
      ...
      const state = {
        active: isActive,
        code: localStorage.getItem("rh-speedticket") || null,
        usage: speedTicketUsage,
        settings: speedTicketSettings,
      }
      ...
      Object.entries(await window.getFrames()).filter(e => {
        ...
        let event = new CustomEvent("speedState");
        event.speedState = state;
        e[1].dispatchEvent(event);
      })
    })
    ...
    const refreshSpeedInfo = async function() {
      const isActive = rhSession.speedTicketActive || false;
